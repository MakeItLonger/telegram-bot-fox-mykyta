...
bot.setMyCommands([
  ...
  {
    command: '/notifynotvoting',
    description: 'Покликати тих хто не голосував',
  },
]);
...
const lastDigest = 'https://t.me/c/1588797053/19';
...
axios
  .get(
    'https://paintedfoxbot-default-rtdb.europe-west1.firebasedatabase.app/poll.json'
  )
  .then((response) => {
...
axios
  .get(
    'https://paintedfoxbot-default-rtdb.europe-west1.firebasedatabase.app/voting.json'
  )
  .then((response) => {
    console.log('Response:', response.data);
    for (let key in response.data) {
      fbVotingID = key;
      arrVotingUsers = response.data[key];
    }
  })
  .catch((error) => {
    console.error('Error:', error);
...
const optionsQuiz = [
  [
    'Початок нового життя після всесвітнього потопу',
    'Вихід з Єгипту',
    'Закінчення будівництва Єрусалимського храму',
    'Визволення з вавилонського полону',
    'Пасха стосується лише Нового Завіту',
  ],
  [
    'Воскресіння Христа',
    'Свято опрісноків',
    'Великдень',
    'Благовіщення',
    'Усі ці свята можна назвати Пасхою!',
  ],
  [
    'Об’єднуватись з іншими сім’ями',
    'Ламати кістки жертовному агнцю',
    'Їсти квасне',
    'Нічого з цього робити не можна було',
  ],
  [
    'Зрада Юдою Ісуса',
    'Ісус омив ноги учням',
    'Перше причастя',
    'Молитва Ісуса в Гефсиманському саду',
    'Усі події відбулися в той день',
  ],
  [
    'Пасхальний кролик',
    'Писанки/Крашанки',
    'Паска',
    'Освячений кошик',
    'Біблія не згадує про жоден з них',
  ],
  ['Жертва', 'Святість', 'Пройти повз', 'Покаятись у гріхах'],
  [
    'Порода жертовної вівці',
    'Ім’я фараона',
    'Назва пасхальної вечері',
    'Напій, який треба було пити разом із гіркими травами',
  ],
  [
    'Ягня було образом, що Ісус зробив для наc, проливши Свою кров заради нас',
    'Не у всіх зараз є худоба дома',
    'Сусіди не оцінять',
  ],
  [
    'Ближче до старості, коли буде сенс думати про Бога',
    'Якомога швидше, доки не стало пізно',
  ],
...
bot.on('new_chat_members', (msg) => {
  ...
  bot.sendMessage(
    chatID,
    `Раді вітати тебе, ${stringMembers}! Господь цінить твоє бажання вивчати Боже Слово. Загальну інформацію ти можеш знайти в описі групи. Якщо тобі потрібна моя допомога, користуйся командами в правому нижньому куті чату - [/] або набирай руцями '/'`
  );
});

bot.on(
  'poll_answer',
  async ({ poll_id, user, option_ids: [answer], option_ids: arrAnswers }) => {
    if (poll_id === quizID) {
    ...
    } else if (poll_id === pollID) {
      if (arrVotingUsers.length === 0 && arrAnswers.length !== 0) {
        arrVotingUsers.push([user.first_name, user.id]);
        axios
          .post(
            'https://paintedfoxbot-default-rtdb.europe-west1.firebasedatabase.app/voting.json',
            arrVotingUsers
          ...
          .then((response) => {
            console.log('Response:', response.data);
            fbVotingID = response.data.name;
          })
          .catch((error) => {
            console.error('Error:', error);
          });
      } else if (arrVotingUsers.length !== 0 && arrAnswers.length !== 0) {
        arrVotingUsers.push([user.first_name, user.id]);
        axios
          .patch(
            'https://paintedfoxbot-default-rtdb.europe-west1.firebasedatabase.app/voting.json',
            {
              [fbVotingID]: arrVotingUsers,
            }
          )
          .then((response) => {
            console.log('Response:', response.data);
        ...
        arrVotingUsers = arrVotingUsers.filter(([name, id]) => id !== user.id);
        axios
          .patch(
            'https://paintedfoxbot-default-rtdb.europe-west1.firebasedatabase.app/voting.json',
            {
...
bot.on('message', async (msg) => {
  ...
  if (text && text.toLowerCase().startsWith('/quiz') && !isQuizLaunched) {
    ...
    for (let i = 0; i < questionsQuiz.length; i++) {
      await bot
        .sendPoll(chatID, `${i + 1}. ${questionsQuiz[i]}`, optionsQuiz[i], {
          is_anonymous: false,
    ...
    const sortedAnswers = Object.fromEntries(
      Object.entries(answers).sort((a, b) => b[1][0] - a[1][0])
    );

    for (let key in sortedAnswers) {
      place++;
      quizResults += `\n${place}. ${key} - ${answers[key][0]}/${
        answers[key][1]
      } (${Math.round((answers[key][0] * 100) / answers[key][1])}%)`;
    }
    ...
    bot.sendMessage(
      chatID,
      'В даний момент є можливість приймати участь тільки в поточному квізі. Якщо ви хочете запустити новий квіз, дочекайтесь результатів поточного. 😌'
    );
  }
  ...
  if (text && text.toLowerCase().startsWith('/poll') && !isPollLaunched) {
    ...
    const [place = 'Церква Благодать', time = '19:00', date = 'в четвер'] =
      optionsArr.map((item) => item.trim());
    ...
    bot.sendMessage(
      chatID,
      'Опитування на цей тиждень вже було створене, наступного разу будьте уважніше! 😉'
    );
  }
  ...
  if (text && text.toLowerCase().startsWith('/notifyall')) {
    ...
    bot
      ...
      .then((admins) => {
        ...
        const strNotifyAll = str.replace('_', ' ');
  ...
  if (text && text.toLowerCase().startsWith('/notifynotvoting')) {
    const arrUsers = [];
    bot
      .getChatAdministrators(chatID)
      .then((admins) => {
        for (let { user } of admins) {
          if (!user.is_bot) {
            arrUsers.push([user.first_name, user.id]);
          }
        }
        const arrUsersID = Object.values(Object.fromEntries(arrVotingUsers));
        const arrUsersInfo = arrUsers.filter(
          ([name, id]) => !arrUsersID.includes(id)
        );
        let str = ``;
        for (let [name, id] of arrUsersInfo) {
          str += `[${name}](tg://user?id=${id}) `;
  ...
  if (
    (/прошу/i.test(text) && /молитис/i.test(text)) ||
    ...
    /моліться/i.test(text) ||
    /молітся/i.test(text) ||
    /молитесь/i.test(text) ||
    /молытесь/i.test(text)
  ) {
    const randomNumber = getRandomNumber();
    bot.sendMessage(
      chatID,
      `Давайте підтримаємо в молитві [${fullName}](tg://user?id=${userID})\\. 🙏\n\n_${prayerEncouragements[randomNumber]}_`,
